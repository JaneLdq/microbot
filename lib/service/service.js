...
const Service = module.exports = class Service extends EventEmitter {
	...
	publish(message, callback) {
		if (this.server.publish) {
			if(this.publishedTopics.indexOf(message.topic) == -1) {
				this.publishedTopics.push(message.topic);
			}
	...
	subscribe(topic, callback) {
		if (!this.subport) {
			throw new Error('No subscribe listening port is set to service ' 
				+ this.name + ', please ensure the\'subport\' property of service configuration is set.' );
		}
		let broker = this.broker;
		let topicName = topic;
		if (_.isObject(topic) && topic.broker) {
			broker = {
				host: topic.broker,
				port: 1883
			};
		...
		_callProxy.call(this, 'mqtt', null, {topic: topicName, port: this.subport, broker: broker}, callback);
	...
	toJSON() {
		let obj = {
			...
			broker: this.broker,
			robots: Tools.listToJSON(this.robots),
			subscriptions: this.subscribedTopics,
			publishments: this.publishedTopics
		}
		if (this.protocol === 'http') {
			obj.routes = this.server.toJSON()
		}
		return obj;
	}
...
function _callProxy(protocol, uri, message, callback) {
	...
	// message: topic, port, broker {host, port}
	let postData = {
		protocol: protocol,
		uri: uri,
...
function _validate(opts) {
	...
	if (opts.protocol === 'mqtt' && !opts.broker) {
		Logger.warning('MQTT service: ' + this.name + ' without a specified broker, will using default broker instead. Default broker\'s details is configurable in file config.js');
	}
}