...
var Service = module.exports = function Service(opts) {
	...
	if (opts.subport)  {
		...
		this._initMqttListener(opts.subport);
	}
};
 ...
 * @param {Function} [callback] function (err, data), data is a JSON object contains message, fired when get a message. An Error occurs if sending request to proxy failed or subscribe failed
 * @return {Object} the service instance
 */
Service.prototype.subscribe = function(topic, callback) {
	this.on(topic, callback);
	this._callProxy('mqtt', null, {topic: topic, port: this.subport}, callback);
	return this;	
};
 ...
 * @return {Object} the service instance
 */
Service.prototype.request = function(uri, params, callback) {
	this._callProxy('http', uri, params, callback);
	return this;
};
 ...
 */
Service.prototype._callProxy = function(protocol, uri, message, callback) {
	var that = this;
	var options = {
	...
	var fnHttpRequestHandler = function(res) {
	...
	var fnMqttRequestHandler = function(res) {
		res.setEncoding('utf-8');
		var data = "";
		res.on('data', function(chunk) {
			data += chunk;
	...
	var req = protocol === 'http' ? http.request(options, fnHttpRequestHandler) : 
...
/**
 * Initialize a httq server to listening mqtt published messages
 * @param {Number} [port] listening port
 * @return {void}
 */
Service.prototype._initMqttListener = function(port) {
	var that = this;
	this.listener = http.createServer(function(req, res) {
		...
		req.on('end', function(res) {
			...
			that.emit(data.topic, null, JSON.parse(data.message));
		});
		req.on('error', function(err) {
			that.emit(data.topic, err);
		})
		res.writeHead(204);
		res.end();
	}).listen(port);
 ...
 */
Service.prototype._validate = function(opts) {
