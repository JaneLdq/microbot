...
var MessageProxy = module.exports = function MessageProxy () {
	...
	this.on('subscribed', function(topic) {
		_.remove(this.pendingTopics, function(value) {
			return value === topic;
		});
		var clients = [];
		_.forEach(this.pendingClients[topic], function(res, key) {
			clients.push(key);
			res.writeHead(204);
			res.end();
		});
...
Utils.inherits(MessageProxy, EventEmitter);
...
MessageProxy.prototype._mqttHandler = function(req, res, data) {
	...
	var fnAddClient = function(map, topic, client, port) {
		var clients = map[topic] || new Array();
	...
	var fnAddPendingReq = function(map, topic, key, res) {
		var reqs = map[topic] || {};
		if (!reqs[key]) {
			reqs[key] = res;
		}
	...
	if (this.topics.indexOf(topic) !== -1) {
		...
		this.mqttAgent.subscribe(topic, null, function(err, granted) {
			if (err) {
				res.writeHead(503);
				res.write(JSON.stringify({message: err.message}));
				res.end();
			} else {
				that.emit('subscribed', granted[0].topic);
			}
		});
	}
};
