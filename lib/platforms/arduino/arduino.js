...
const Arduino = module.exports = class Arduino extends Adaptor {
	...
	connect(callback) {
		this._board = new Board(this.port, (err) => {
			if (err) {
				callback(err);
			}
	 ...
	 * @return {void}
	 */
	pinMode(pin, mode = this._board.MODES.INPUT) {
	...
	analogWrite(pin, value) {
		...
		this.pinMode(this._board.analogPins[pin], this._board.MODES.ANALOG);
