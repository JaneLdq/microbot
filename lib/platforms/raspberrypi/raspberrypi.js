...
const Raspberrypi = module.exports = class Raspberrypi  extends Adaptor{
	...
	connect(callback){
		// this._proxyMethods(this.commands,this.board,this);
...
console.log("in raspi adaptor digitalread");
		let pin = this.pins[this._translatePin(pinNum)];
		
		if(pin == null){
			pin = this._digitalPin(pinNum,"r");
			
			pin.on("digitalRead", (val) => {
				this._respond("digitalRead", callback, null , val, pinNum);
			});
	...
	digitalWrite(pinNum,value,callback){
		...
		if(pin != null){
			...
			pin.on("digitalWrite", (val) => {
				this._respond("digitalWrite", callback, null, val, pinNum);
			});
	 ...
	 *
	 * @param {Number} address I2C address to read from
	 * @param {Number} cmd I2C command to write
